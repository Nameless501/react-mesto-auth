        ...
        <section className={`popup popup_type_${name} ${isOpen && "popup_opened"}`}>
            <form name={name} className="popup__form" onSubmit={onSubmit} noValidate>
                <fieldset className="popup__fieldset">
                    ...
                    {children}
                    <button type="submit" className={`popup__submit-button ${name === 'delete' && 'popup__submit-button_place_delete-popup'} ${!isValid && "popup__submit-button_disabled"}`} disabled={!isValid} >
